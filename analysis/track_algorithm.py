...
def get_best_rec(song, artist, debug=False):
    ...
    sentiments = get_sentiments(song=song, artist=artist)
    ...
    for candidate in candidates:
        ...
        cand_sent = get_sentiments(song=candidate['song'], artist=candidate['artist'])
        cand_feat = get_audio_features(song=candidate['song'], artist=candidate['artist'])
        if debug:
            print(sentiments)
            print(cand_sent)
            print(features)
    ...
    return candidate_map[0], sentiments[0]
